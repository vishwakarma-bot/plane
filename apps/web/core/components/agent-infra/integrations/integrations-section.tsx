...
import { useCatalogIntegrations } from "@/hooks/use-catalog";
import { IntegrationsList } from "./integrations-list";

type TIntegrationsSectionProps = {
  workspaceSlug: string;
  projectId: string;
};

export function IntegrationsSection(props: TIntegrationsSectionProps) {
  const { workspaceSlug, projectId } = props;
  const { t } = useTranslation();
  const { integrations, isLoading, error } = useCatalogIntegrations(workspaceSlug, projectId);
