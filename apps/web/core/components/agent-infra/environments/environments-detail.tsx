...
function renderListItems(items?: string[]) {
  ...
  return (
    <ul className="space-y-1.5">
      {items.map((item) => (
        <li key={item} className="rounded-sm bg-layer-2 px-3 py-1.5 font-mono text-12 text-secondary">
          {item}
        </li>
      ))}
    </ul>
    ...
    <div className="rounded-lg border border-subtle bg-surface-1 p-5">
      ...
      <dl className="mt-6 grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <dd className="mt-1 font-mono text-12 text-secondary">{environment.path}</dd>
        </div>
        <div>
          <dt className="text-11 font-medium uppercase tracking-wide text-tertiary">
            {t("agent_infra.environments_tab.snapshot")}
          </dt>
          <dd className="mt-1 font-mono text-12 text-secondary">{truncateContentHash(environment.content_hash)}</dd>
        </div>
      </dl>
